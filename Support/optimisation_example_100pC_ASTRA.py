...
def optFuncVELA(names, values):
    ...
    # try:
        ...
        constraintsListBA1 = {
            ...
            'ip_etax': {'type': 'lessthan', 'value': abs(twiss['eta_x_beam'][ipindex]), 'limit': 1e-3, 'weight': 50},
            'ip_etaxp': {'type': 'lessthan', 'value': abs(twiss['eta_xp_beam'][ipindex]), 'limit': 1e-3, 'weight': 50},
        ...
        if delta < bestdelta:
            ...
            OM.export_yaml(auto=False, filename='optimise_100pC_ASTRA.yaml', directory='.')
...
if __name__ == '__main__':
    OM.import_yaml(filename='./optimise_100pC_ASTRA.yaml')
    OM.modify_widget('Linac:tracking_code:value', 'ASTRA')
    ...
    # OM.modify_widget('Linac:CLA-L01-CAV:phase', -10)
    quad_names = [
